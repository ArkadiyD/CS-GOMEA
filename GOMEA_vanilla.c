...
char       *elitist_solution,                                       /* The very best solution ever evaluated. */
         ...
         ***offsprings,                                             /* Offspring solutions (one set per population). */
           *terminated;                                             /* Whether a specific GOMEA with the restart scheme has terminated. */
...
int max_evaluated_solutions = 500000;
char **evaluated_solutions;
double *evaluated_archive;
int evaluated_archive_size = 0;
...
char *generateAndEvaluateNewSolutionSpecificGOMEA( int gomea_index, int parent_index, double *obj, double *con );
...
double checkAlreadyEvaluated(char *parameters);
...
double checkAlreadyEvaluated(char *parameters)
{
  ...
  for (int i = start; i < evaluated_archive_size; ++i)
  {
    bool matched = true;
    ...
    if (matched) return evaluated_archive[i];
  }
  return -1;
}
...
void addToEvaluated(char *parameters, double objective_value)
{
  if (checkAlreadyEvaluated(parameters) >= 0)
    return;

  if (evaluated_archive_size >= max_evaluated_solutions)
    return;

  for (int i = 0; i < number_of_parameters; ++i)
  {
      evaluated_solutions[evaluated_archive_size][i] = parameters[i];
  }
  evaluated_archive[evaluated_archive_size] = objective_value;
  //printf("%d %d\n", number_of_evaluations, evaluated_archive_size);
...
void parseParameters( int argc, char **argv, int *index )
{
  ...
  noError = noError && sscanf( argv[*index+5], "%s", folder_name );
...
void installedProblemEvaluation( int index, char *parameters, double *objective_value, double *constraint_value, int number_of_touched_parameters, int *touched_parameters_indices, char *parameters_before, double objective_value_before, double constraint_value_before )
{
  ...
  *constraint_value = 0.0;
  ...
  if( (number_of_evaluations == 1) || betterFitness( *objective_value, *constraint_value, elitist_solution_objective_value, elitist_solution_constraint_value ) )
  {
    ...
    //writeRunningTime( (char *) "output/elitist_solution_hitting_time.dat" );
    writeElitistEvaluations((char *) filename_elitist_solutions, elitist_solution_objective_value, elitist_solution_constraint_value);
...
void initializeNewGOMEA()
{
  if( number_of_GOMEAs == 0 )
  {
    ...
    population_sizes[number_of_GOMEAs] = (int)(2*population_sizes[number_of_GOMEAs-1]);
...
void initializeNewGOMEAPopulationAndFitnessValues()
{
  ...
  //printf("current size %d", population_sizes[number_of_GOMEAs]);
  for( i = 0; i < population_sizes[number_of_GOMEAs]; i++ )
  {
    for( j = 0; j < number_of_parameters; j++ )
...
short initializeValueToReach()
{
  ...
  sprintf( filename, "%s/vtr.txt", folder_name );
  file = fopen( filename, "r" );
  if( file == NULL )
    return( 0 );
...
short initializeSetOfSolutionsToReach()
{
  ...
  sprintf( filename, "%s/sostr.txt", folder_name );
  file = fopen( filename, "r" );
  if( file == NULL )
    return( 0 );
...
void writeGenerationalStatistics()
{
  ...
  /* Then write them */
  sprintf( filename, "%s/vanilla_statistics.dat", folder_name );
  file = NULL;
...
void writeGenerationalSolutions( char is_final_generation )
{
  ...
  if( is_final_generation )
    sprintf( filename, "%s/vanilla_populations_generation_final.dat", folder_name );
  else
    sprintf( filename, "%s/vanilla_populations_generation_%05d.dat", folder_name, number_of_generations );
  file = fopen( filename, "w" );
  ...
  {
    sprintf( filename, "%s/vanilla_offsprings_generation_%05d.dat", folder_name, number_of_generations-1 );
    file = fopen( filename, "w" );
...
void writeElitistEvaluationsInit( char *filename )
{
  ...
  file = fopen( filename, "w" );
  sprintf( string, "Evaluations Objective Constraint\n");
  fputs( string, file );
  fclose( file );
}
...
void writeElitistEvaluations( char *filename, double elitist_objective, double elitist_constraint )
{
  ...
  file = fopen( filename_elitist_solutions, "a" );

  sprintf( string, "%ld %f %f\n", number_of_evaluations, elitist_objective, elitist_constraint );
...
void run()
{
  sprintf(filename_elitist_solution_hitting_time, "%s/vanilla_elitist_solution_hitting_time.dat", (char * )folder_name);
  sprintf(filename_elitist_solutions, "%s/vanilla_elitist_solutions.dat", (char * )folder_name);
  sprintf(filename_elitist_solution, "%s/vanilla_elitist_solution.dat", (char * )folder_name);
  sprintf(filename_solutions, "%s/vanilla_solutions.dat", (char * )folder_name);
...
void multiPopGOMEA()
{
  ...
  base_population_size                      = 8;//1+(int) (6.786843*pow((double)number_of_parameters,1.678253e-01));
