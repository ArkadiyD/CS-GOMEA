...
def run_gomea(call_, elitists_filename, prefix_results, run_count = 0):
    ...
    for run in range(FIRST_RUN, N_RUNS):
        ...
        if 'surrogate' in prefix_results:
            time_arr = np.array([time_for_run], dtype=np.int32)
            np.savetxt('%s/%s_time.dat' % (FOLDER_NAME_RUN, prefix_results), time_arr)
...
gomea_vanilla = ['./GOMEA_vanilla', '-v', str(PROBLEM_NUMBER), str(
    DIMENSIONALITY), str(FOS), str(1000000), '-1', FOLDER_NAME]
run_gomea(gomea_vanilla, 'vanilla_elitist_solutions.dat', 'vanilla')
